...
locals {
  default_target_url = "${var.service_name}${var.environment_number}.*"
  log_options_aws = {
    awslogs-group         = "/ecs/${var.service_name}-task-def-${var.environment}"
    awslogs-region        = "eu-west-1"
    awslogs-stream-prefix = "${var.environment}/${var.container_name}/${var.service_name}_task_def"
  }
  log_options_splunk = {
    Name             = "splunk"
    ...
    tls              = "On"
    event_sourcetype = var.service_name
