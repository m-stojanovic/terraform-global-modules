...
variable "cross_account_validation" {
  description = "If true, skip the ACM module's built-in Route53 record creation and do cross account validation ourselves."
  type        = bool
  default     = false
}

variable "cross_account_zone_id" {
  description = "Hosted Zone ID in the other account (used only if cross_account_validation = true)."
  type        = string
  default     = ""
}
