locals {
  ...
  # We only create cross-account R53 records if var.cross_account_validation is true.
  # Key the map by resource_record_name, so duplicates are dropped automatically.
  unique_validation_map = var.cross_account_validation ? {
...
resource "aws_route53_record" "cross_account_validation" {
  count = var.cross_account_validation && var.validation_method == "DNS" && var.create_route53_records ? length(local.distinct_domain_names) : 0

  provider = aws.r53_zone_owner
  zone_id  = var.cross_account_zone_id
  name     = element(local.validation_domains, count.index)["resource_record_name"]
  type     = element(local.validation_domains, count.index)["resource_record_type"]
  ttl      = var.dns_ttl
...
resource "aws_acm_certificate_validation" "cross_account_validation" {
  count = var.cross_account_validation ? 1 : 0

  certificate_arn = aws_acm_certificate.this[0].arn

  validation_record_fqdns = flatten([aws_route53_record.cross_account_validation[*].fqdn, var.validation_record_fqdns])
